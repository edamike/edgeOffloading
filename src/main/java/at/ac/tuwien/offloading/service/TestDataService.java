...
public class TestDataService {
    ...
    private void findAllEuaServersWithinDistance(Location loc, List<EuaEdgeServer> allServers, double maxDistance, EuaUser temp) {
        ...
        allServers.stream().filter(a -> a.getDistance() <= maxDistance).forEach(i -> temp.setDistance(i.getDistance()));
        allServers.stream().filter(a -> a.getDistance() <= maxDistance).forEach(i -> i.addEuaUserList(temp));
    }
