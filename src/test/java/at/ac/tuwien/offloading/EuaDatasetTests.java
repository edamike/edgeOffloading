...
public class EuaDatasetTests {
    ...
    public void testPrediction() throws IOException {
        ...
        for(int t = 1; t <= 10; t++)
        {
            predictMobility(uppaalRequest, erg, a, t, maxDistance, sheet, devices, edges);
        }
    ...
    public void testTime() throws IOException {
        ...
        int i = 1;
        for(int deviceSize=10; deviceSize < 350; )
        {
            ...
            double staticResources = makeAnalysis(uppaalRequest);
    ...
    public void makeTest(Row row, int deviceSize) throws IOException {
        ...
        uppaalRequest.setStrategyEnum(Strategy.PERIODIC);
        double periodicResources = makeAnalysis(uppaalRequest);
        ...
        double unpredictableResources = makeAnalysis(uppaalRequest);
    ...
    private double makeAnalysis(UppaalRequest uppaalRequest)
    {
        uppaalRequest.setComputationIntensity(ComputationIntensity.LOW);
        uppaalRequest.setRequiredRuns(50);
        uppaalRequest.setConsideredTimeUnits(10);
        ...
        {
            maxUsedResources += Double.parseDouble(n.getMessage().substring(0,
    ...
    private void makeTestSingleEdge(Row row, int deviceSize)
    {
        ...
        {
            double maxUsedResources = Double.parseDouble(erg.getEdgeNodes().get(0).getMessage().substring(0,
                    erg.getEdgeNodes().get(0).getMessage().indexOf('±')));
            if(i==1) {
                LOG.info("STATIC");
    ...
    private void predictMobility(UppaalRequest uppaalRequest, UppaalResponse erg, double a, int t, double maxDistance, Sheet sheet, Sheet devices, Sheet edges) {
        ...
        testUtil.logErrorsToExcel(rows, errors, sheet, t, offloadings);
    }
}
